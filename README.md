...
高端化车型：类别：{60,120};{110,300}；均为球形震动手柄。

```pascal
（*参数初始化zat1100v7） g_p:=10886/g_lb/0.02; g_ly:=(2.335*g_lb*g_lb - 277.7*g_lb + 1.029E4) *1E-6;
（*参数初始化zat3000v7） g_p:=21516/g_lb/0.02; g_ly:=(3.98*g_lb*g_lb - 564.5*g_lb + 2.49E4) *1E-6;
（*参数初始化zat1200v7） g_p:=12472/g_lb/0.02; g_ly:=(2.395*g_lb*g_lb - 292.8*g_lb + 1.131E4) *1E-6;
（*参数初始化zat600v6）  g_p:=6465/g_lb/0.02;  g_ly:=(1.729*g_lb*g_lb - 156*g_lb + 4.53E3) *1E-6;
```

60v6:

- 60v6e，改周期组发送。60v6z，未改它部周期组发送。
  - 测试后控制器发送双频率的 id 有 18A、472、489、289，另外力限器发送的 48F 的频率也是变动的。

- 落钩跳出随动（有 `autowinch_error`）。球形手柄难以保持横向扳动时纵向值为零（在死去内）。

- 参数281=2 切换无挡位的回转电流（回转最大电流不再接受力限器常规参数），防摇摆可以不改。

- `overlaod_overroll_overfang_control` 中，EN13000标准版不再写入主臂模式的变量。

- `anti_slew_angle`已被逆时针占用，防摇摆使用先被`36000`减去；回转角度获取=关，则为回转编码器。

120v7e:

- 负责人改周期组发送。
- 落钩跳出随动（有 `autowinch_error`）。球形手柄难以保持横向扳动时纵向值为零（在死区内）。

110v7g/e:

- 改周期组发送。
- 落钩不跳出随动（无 `autowinch_error`），只是不响应。球形手柄难以保持横向扳动时纵向值为零（在死区内）。

300v7g/e:

- 300g为非ce迈新，300e为ce的迈新。
- 落钩跳出随动（有 `autowinch_error`），或不响应。球形手柄难以保持横向扳动时纵向值为零（在死区内）。

2025年8月26日：发现400-1 与 300e 的高度补偿 retain 变量未纳入正确写法，350 有它易混淆，300g不在数组。
